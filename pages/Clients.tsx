...
import { Search, Plus, Edit, Trash2, X, DollarSign, UserCheck, AlertCircle, Printer } from 'lucide-react';
    ...
    <div className="p-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
            <div>
                ...
                <div className="flex flex-col text-xs text-gray-500">
                    <span>Bs. {(totalDebt * config.exchangeRate).toFixed(2)}</span>
                    {config.showCop && <span>COP {(totalDebt * config.copExchangeRate).toLocaleString('es-CO')}</span>}
                </div>
            </div>
      ...
      <div className="flex justify-between items-center mb-6 no-print">
        ...
        <div className="flex gap-2">
            ...
            {canEdit && (
                <button 
                onClick={() => { setEditingClient(null); setFormData(initialFormState); setIsFormOpen(true); }}
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-100">
                ...
                return (
                  <tr key={client.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4">
                        ...
                        <div className="text-xs text-gray-500">Bs. {(client.debt * config.exchangeRate).toFixed(2)}</div>
                        {config.showCop && <div className="text-xs text-gray-400">COP {(client.debt * config.copExchangeRate).toLocaleString('es-CO')}</div>}
                    </td>
              ...
              {filteredClients.length === 0 && (
                 <tr>
                    <td colSpan={6} className="p-8 text-center text-gray-400">No se encontraron clientes</td>
                 </tr>
              )}
            </tbody>
          </table>
      ...
      {/* Create/Edit Client Modal (No Print) */}
      {isFormOpen && (
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full">
            ...
            <form onSubmit={submitForm} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input required type="text" className="w-full p-2 border rounded-lg" value={formData.identityCard} onChange={e => setFormData({...formData, identityCard: e.target.value})} placeholder="V-12345678" />
              </div>
              <div>
              ...
              <div>
                ...
                <input required type="number" className="w-full p-2 border rounded-lg" value={formData.creditLimit} onChange={e => setFormData({...formData, creditLimit: parseFloat(e.target.value)})} />
              </div>
              
              <div className="flex gap-4 pt-4">
                <button type="button" onClick={() => setIsFormOpen(false)} className="flex-1 py-2 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg">Cancelar</button>
            ...
            <form onSubmit={submitPayment} className="p-6 space-y-6">
              <div className="text-center">
                ...
                <div className="text-sm text-gray-500">
                    <span>Bs. {(payingClient.debt * config.exchangeRate).toFixed(2)}</span>
                    {config.showCop && (
                        <>
                            <span className="mx-2">•</span>
                            <span>COP {(payingClient.debt * config.copExchangeRate).toLocaleString('es-CO')}</span>
                        </>
                    )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                    ...
                    <input 
                        ...
                        type="number" 
                        step="0.01" 
                        max={payingClient.debt}
                        className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 text-lg font-semibold" 
                        value={paymentAmount} 
              ...
              <div className="flex gap-4">
                <button type="button" onClick={() => setIsPaymentOpen(false)} className="flex-1 py-2 text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg">Cancelar</button>
                <button type="submit" className="flex-1 py-2 bg-green-600 text-white font-medium hover:bg-green-700 rounded-lg">Confirmar Pago</button>
              </div>
            </form>
          </div>
        </div>
